...
public interface IRecipeDumper<T extends Recipe<?>> {
    ...
    default void setOutputs(T recipe, IRecipeOutputs outputs, HolderLookup.Provider registries) {
        outputs.addOutput(1, assembleResult(recipe));
    }
    ...
    private static ItemStack assembleResult(Recipe<?> recipe) {
        if (recipe instanceof SingleItemRecipe singleItemRecipe) {
            return singleItemRecipe.assemble(new SingleRecipeInput(sampleStack(singleItemRecipe.input())));
        }
        if (recipe instanceof ShapedRecipe shapedRecipe) {
            ...
            return shapedRecipe.assemble(CraftingInput.of(shapedRecipe.getWidth(), shapedRecipe.getHeight(), items));
        }
        if (recipe instanceof ShapelessRecipe shapelessRecipe) {
            ...
            return shapelessRecipe.assemble(CraftingInput.of(Math.max(1, items.size()), 1, items));
        }
